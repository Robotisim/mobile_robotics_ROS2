...
https://docs.ros.org/en/humble/Install
ation/Ubuntu-Install-Debians.html#install-ros-2-packages

Step#2 ESP IDF Installation 
Esp IDF extention using VSCode:
https://github.com/espressif/vscode-esp-idf-extension/blob/master/docs/tutorial/install.md
try different examples like (Hello World,Talker listner)
It can be done on VScode by adding the examples by {View-Command Pallet -"select the Esp Idf Examples"} then create the project using the examples in specific workspace OR using Terminal.
...
cd microros_ws: The "cd" command is short for "change directory." It is used to change the current working directory to the specified directory.
...
Above command requires a password to install.
apt-get: This is a package management tool used in Debian-based Linux distributions (e.g., Ubuntu) to install, upgrade, and remove software packages.


# Building packages and setting up your workspace to use the built packages:
...
Step#3.1 Creating a new firmware workspace for ESP32
The command is used to create a new firmware workspace for micro-ROS on an ESP32 microcontroller running FreeRTOS as the Real-Time Operating System (RTOS).After executing this command a folder called "firmware" will be created in the workspace along with the required code for building micro-ros app.
...
**** INT32 publisher example ****
Step#3.2 Configuring Micro-ros firmware:
following command will help configure the files for our project,enter the project name which will include the code and Device IP address which could be either(computer, laptop etc)
...
Below command can tell you the IP address of the devise
ifcongig
...
Step#3.3 Configure Wi-Fi Credentials:
...
Step#3.4 Building micro Ros firmware:
...
Step#3.5 Flashing Micro Ros firmware:
...
Step#3.6 Building Micro Ros Agent:
why Building micro-ROS-Agent is necessary
#1
Building the micro-ROS-Agent is necessary because it serves as a crucial bridge between your embedded system (microcontroller or resource-constrained device) and the ROS 2 ecosystem. The micro-ROS-Agent enables communication and interaction between these two environments, allowing your embedded device to become a part of the ROS 2 network and leverage the capabilities of ROS 2.
#2
...
source install/local_setup.sh
#Run micro ros agent:
Once ESP32 connected to the Wi-Fi network and the micro-ROS-Agent running with the correct parameters, the ESP32 should be able to communicate wirelessly with the ROS 2 ecosystem via micro-ROS.
ros2 run micro_ros_agent micro_ros_agent [parameters]
